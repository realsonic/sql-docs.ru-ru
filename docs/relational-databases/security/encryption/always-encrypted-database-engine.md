...
Шифрование и расшифровка осуществляются с помощью драйвера клиента. Это означает, что некоторые действия, которые происходят только на стороне сервера, при использовании Always Encrypted работать не будут. Примеры включают копирование данных из одного столбца в другой с помощью операций UPDATE, BULK INSERT(T-SQL), SELECT INTO, INSERT..SELECT. 
...
> [!NOTE]
> [Always Encrypted с безопасными анклавами](always-encrypted-enclaves.md), представленными в [!INCLUDE[sql-server-2019](../../../includes/sssqlv15-md.md)], поддерживает шифрование существующих данных с помощью Transact-SQL. Это также избавляет от необходимости перемещать данные за пределы хранилища для криптографических операций.
...
-   Запросы для столбцов, зашифрованных с помощью случайного шифрования, не могут выполнять никаких операций. Индексирование столбцов, зашифрованных с помощью случайного шифрования, не поддерживается.  
 > [!NOTE] 
 > [Always Encrypted с безопасными анклавами](always-encrypted-enclaves.md), представленными в [!INCLUDE[sql-server-2019](../../../includes/sssqlv15-md.md)], устраняет описанное выше ограничение, включая сопоставление шаблонов, операторы сравнения и индексирование столбцов с использованием случайного шифрования.
