...
|waittype|**двоичный (2)**|Зарезервировано.|  
...
|sql_handle|**двоичный (20)**|Представляет пакет или объект, который выполняется в настоящий момент.<br /><br /> **Примечание** . Это значение берется из пакета или адреса памяти объекта. Оно не вычисляется с помощью алгоритма [!INCLUDE[ssNoVersion](../../includes/ssnoversion-md.md)] на основе хэша.|  
