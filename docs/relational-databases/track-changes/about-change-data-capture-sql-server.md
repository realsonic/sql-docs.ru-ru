...
Использование переменных для переключения секций в базах данных или таблицах с системой отслеживания измененных данных (CDC) не поддерживается для инструкции `ALTER TABLE ... SWITCH TO ... PARTITION ...`. Дополнительные сведения см. в разделе об [ограничения переключения секций](../replication/publish/replicate-partitioned-tables-and-indexes.md#replication-support-for-partition-switching). 
