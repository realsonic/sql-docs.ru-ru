...
- **Формат файла**. Указывает формат файла источника. Поддерживаемые форматы: **текст**, **Avro**, **ORC**, **Parquet**. Java требуется для ORC/Parquet. Подробные сведения см. [здесь](../../integration-services/azure-feature-pack-for-integration-services-ssis.md#dependency-on-java).
